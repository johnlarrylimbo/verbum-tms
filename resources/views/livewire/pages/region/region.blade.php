<div x-data="{ init: false }" x-init="if (!init) { init = true; $wire.region_lst() }">
  <x-mary-header title="SystemLib :: Region">
      <x-slot:middle class="!justify-end">
          <x-mary-input icon="o-magnifying-glass" placeholder="Search Region..."  wire:model.live="search"/>
      </x-slot:middle>
      <x-slot:actions>
          <x-mary-button label="Create" icon="m-folder-plus" class="btn-primary" @click="$wire.addRegionModal = true" />
      </x-slot:actions>
  </x-mary-header>
      ...
      <table class="table mb-4 table-striped w-full" style="table-layout: fixed; min-width: 600px;">
        <thead>
          <tr class="fs-14 pink h-2rem">
            ...
            <th class="text-center bg-primary text-white" width="10%">Short Code</th>
        ...
        <tbody>
          @if(count($this->region_lst) == 0)
            <tr class="fs-13 border-btm content-tr">
              <td class="text-center" colspan="6">No region record(s) found.</td>
            </tr>
          @else
            @foreach ($this->region_lst as $result)
              <tr class="fs-13 border-btm content-tr">
                <td class="align-top text-center">{{ $result->row_num }}</td>
                <td class="text-center vertical-align-top" style="word-break: break-word;">{{ $result->numerals }}</td>
                ...
                <td class="text-center vertical-align-top">
                  <x-mary-button icon="o-pencil-square" 
                                  wire:click="openEditRegionModal({{ $result->region_id }})" 
                                  wire:target="openEditRegionModal"
                                  spinner 
                                  class="bg-green-600 text-white btn-sm align-center" />&nbsp;
                  @if($result->statuscode == 1)
                    <x-mary-button icon="o-eye-slash"
                                    wire:click="openUpdateRegionStatusModal({{ $result->region_id }},{{ $result->statuscode }})"
  ...
  <!-- 
    Loader goes here 
  -->
  <x-livewire-loader target="region_lst" message="Please wait while the system loads all region records for you..." />
