<div>
  <x-mary-header title="SystemLib :: Contract Type">
      ...
      <x-slot:actions>
          <x-mary-button label="Create" icon="m-folder-plus" class="btn-primary" @click="$wire.addContractTypeModal = true" />
      </x-slot:actions>
  </x-mary-header>
      ...
      <table class="table mb-4 table-striped w-full" style="table-layout: fixed; min-width: 600px;">
        <thead>
          <tr class="fs-14 pink h-2rem">
            ...
            <th class="text-center bg-primary text-white" width="35%">Contract Type Description</th>
            <th class="text-center bg-primary text-white">Status</th>
            <th class="text-center bg-primary text-white">Manage</th>
          </tr>
        </thead>
        <tbody>
          @if(count($this->contract_type_lst) == 0)
            <tr class="fs-13 border-btm content-tr">
              <td class="text-center" colspan="4">No contract type record(s) found.</td>
            </tr>
          @else
            @foreach ($this->contract_type_lst as $result)
              <tr class="fs-13 border-btm content-tr">
                ...
                <td class="text-center vertical-align-top">
                  ...
                  @if($result->statuscode == 1)
                    <x-mary-button icon="o-eye-slash"
                                    wire:click="openUpdateContractTypeStatusModal({{ $result->contract_type_id }},{{ $result->statuscode }})"
  ...
  <x-mary-modal wire:model="editContractTypeModal" class="backdrop-blur custom-modal top-modal">
    <!-- Manual Header -->
    <div class="px-6 pt-4 pb-2 border-b border-gray-200 custom-modal-header-div">
    ...
    <!-- Modal Form -->
    <x-mary-form wire:submit.prevent="save_contract_type_record_changes" no-separator>

      <x-mary-input type="hidden" wire:model="contract_type_id" id="contract_type_id" />
  ...
  <x-mary-modal wire:model="updateContractTypeStatusModal" class="backdrop-blur" title="Please Confirm Action?" separator>
